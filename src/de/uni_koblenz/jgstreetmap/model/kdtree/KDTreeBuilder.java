...
public class KDTreeBuilder {
	public static void buildTree(AnnotatedOsmGraph g, int maxLevels) {
		...
		KDTree kdTree = g.getKdRoot();
		kdTree.setLevels(maxLevels);		
		kdTree.addRoot(constructkdTreeY(g, nodeList, 1));
	}
	
	public static YKey constructkdTreeY(AnnotatedOsmGraph g, LinkedList<Node> nodeList, int depth){
		Collections.sort(nodeList, new YComparator());
		YKey yk = g.createYKey();
		yk.setKeyValue(nodeList.get(nodeList.size()/2-1).getLatitude());
		
		//if the depth of the tree is equal to the number of levels which are allowed
		//exist or the list of nodes which would be splitted contains only one node,
		//NodeSet is assigned to YKey as the child of YKey
		if((depth==g.getKdRoot().getLevels())||(nodeList.size()<=1)){
			NodeSet set=g.createNodeSet();
			set.getElementList();
			yk.addSet(set);
		}
		else{
			//List of nodes is split into two lists, the nodes contained in leftList
			//will be part of the left subtree, the nodes left in nodeList will be
	...
	public static XKey constructkdTreeX(AnnotatedOsmGraph g,LinkedList<Node> nodeList, int depth){
	...
	public LinkedList<Node> rangeQuery(AnnotatedOsmGraph g,double topLeftLong,double topLeftLat,double bottomRightLong, double bottomRightLat,YKey key){
		LinkedList<Node> includedNodes=new LinkedList<Node>();
		double keyVal=key.getKeyValue();
		if(key.getChildList().size()==0){ //is true whenever key has no XKey children,
			//therefore the only child of key is a NodeSet
			rangeQuery(g,topLeftLong, topLeftLat,bottomRightLong,bottomRightLat,key.getSetList().get(0));
		}
		else{
			if(keyVal<bottomRightLat){
				//only the right subtree has to be examined further
				includedNodes.addAll(rangeQuery(g,topLeftLong,topLeftLat,bottomRightLong,bottomRightLat,key.getChildList().get(1)));
			}
			if(keyVal>=topLeftLat){
				//only the left subtree has to be examined further
				includedNodes.addAll(rangeQuery(g,topLeftLong,topLeftLat,bottomRightLong,bottomRightLat,key.getChildList().get(0)));
			}
	...
	public LinkedList<Node> rangeQuery(AnnotatedOsmGraph g,double topLeftLong,double topLeftLat,double bottomRightLong, double bottomRightLat,XKey key){
		LinkedList<Node> includedNodes=new LinkedList<Node>();
		double keyVal=key.getKeyValue();
		
		if(key.getChildList().size()==0){ //is true, when key has a NodeSet as a child
			rangeQuery(g,topLeftLong, topLeftLat,bottomRightLong,bottomRightLat,key.getSetList().get(0));
		}
		else{
	...
	/**
	 * examines for every Node of the given NodeSet set if the Node is contained by 
	 * the rectangle defined by its vertex at the upper left corner and its vertex
	 * at the lower right corner
	 * @param g
	 * @param topLeftLong longitude of the vertex at the upper left corner of the rectangle
	 * @param topLeftLat latitude of the vertex at the upper left corner of the rectangle
	 * @param bottomRightLong longitude of the vertex at the lower right corner of the rectangle
	 * @param bottomRightLat latitude of the vertex at the lower right corner of the rectangle
	 * @param set 
	 * @return a List of all Nodes which are included in the rectangle
	 */
	public LinkedList<Node> rangeQuery(AnnotatedOsmGraph g,double topLeftLong,double topLeftLat,double bottomRightLong, double bottomRightLat,NodeSet set){
